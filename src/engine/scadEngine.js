...
// // ═══════════════════════════════════════════════════════════════
// // CSG HELPERS
// // three-csg-ts API: CSG.subtract(meshA, meshB) → THREE.Mesh
// //                   CSG.intersect(meshA, meshB) → THREE.Mesh
// // Both meshes must have matrixWorld updated before passing.
// // ═══════════════════════════════════════════════════════════════

// // Walk object tree to find first Mesh — used to extract mesh from wrapped groups
// function extractMesh(obj, visited = new Set()) {
//   if (!obj || visited.has(obj)) return null;
//   visited.add(obj);
//   if (obj.isMesh) return obj;
//   for (const child of obj.children) {
//     const found = extractMesh(child, visited);
//     if (found) return found;
//   }
//   return null;
// }

// // Prepare a mesh for CSG: update matrixWorld, clean normals, return clone
// function prepareMesh(obj) {
//   obj.updateMatrixWorld(true);
//   const mesh = extractMesh(obj);
//   if (!mesh) return null;
//   mesh.updateMatrixWorld(true);
//   // CSG requires clean vertex normals
...
//   try {
//     const baseMesh = prepareMesh(children[0]);
//     if (!baseMesh) throw new Error("Base has no mesh");

//     let result = baseMesh;
//     for (let i = 1; i < children.length; i++) {
//       const toolMesh = prepareMesh(children[i]);
//       if (!toolMesh) continue;
//       result = CSG.subtract(result, toolMesh); // returns THREE.Mesh
//     }
...
//       const toolMesh = prepareMesh(children[i]);
//       if (!toolMesh) continue;
//       result = CSG.intersect(result, toolMesh); // returns THREE.Mesh
//     }
